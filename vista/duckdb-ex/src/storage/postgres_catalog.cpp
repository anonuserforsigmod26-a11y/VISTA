...
void PostgresCatalog::Initialize(bool load_builtin) {
	Connection con(this->GetDatabase());

	//! [VISTA] Initialize the PostgresCatalog here.
	con.BeginTransaction();
	schemas.LoadEntries(*con.context);
	schemas.Scan(*con.context, [&](CatalogEntry &entry) {
		if (entry.type == CatalogType::SCHEMA_ENTRY) {
			auto &schema_entry = entry.Cast<PostgresSchemaEntry>();
			//! [VISTA] Because we do not use composite types or indexes, only the tables are loaded here.
			schema_entry.GetCatalogSet(CatalogType::TABLE_ENTRY)
						.LoadEntries(*con.context);
		}
	});
	con.Commit();
}
