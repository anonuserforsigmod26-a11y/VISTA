...
typedef enum BufferPoolId
{
	ID_SPECIAL = -1,
	POOL_META = 0,	/* metadata pool */
	POOL_OLTP1 = 1,	/* OLTP pool 1 */
	POOL_OLTP2 = 2,	/* OLTP pool 2 */
	// POOL_OLAP = 3,	/* OLAP pool */
	POOL_CHECK_OLTP = 3 /* Used to signal to use OLTP pool, undecided between 1 and 2. */
} BufferPoolId;
...
// extern PGDLLIMPORT uint32 private_seg_refcount[N_POOLS][2];
extern PGDLLIMPORT uint8 *seg_refcount[N_POOLS]; // 0-> unset, 1->pinned during non-flushing, 2->pinned during flushing
/* in freelist.c */
// extern PGDLLEXPORT BufferStrategyControl *vista_StrategyControl[N_POOLS];

/* in vista_snapshot.c */
extern PGDLLIMPORT uint32 vista_olap_generation; // 0 is an invalid generation no.
...
( \
	AssertMacro((bufnum) <= N_POOLS * NBuffers && (bufnum) >= -NLocBuffer), \
...
( \
	...
	: \
		(Block) (vista_BufferBlocks[vista_PoolId(buffer - 1)] + ((Size) vista_SlotId((buffer) - 1)) * BLCKSZ) \
)
#else
#define BufferGetBlock(buffer) \
( \
 ...
 */
#ifdef VISTA
extern PrefetchBufferResult vista_PrefetchSharedBuffer(BufferPoolId poolId, struct SMgrRelationData *smgr_reln,
												 ForkNumber forkNum,
...
#ifdef VISTA
extern bool vista_BgBufferSync(struct WritebackContext *wb_context, BufferPoolId poolId);
#else
...
#ifdef VISTA
extern void reset_buf_table(BufferPoolId poolId);

/* vista_snapshot.c */
extern int vista_StoreGenerationSnapshot(uint32 generation);
extern int vista_LoadGenerationSnapshot(uint32 generation);
extern Size SnapshotShmemSize(void);
extern void vista_InitSnapshotHash(void);
extern void vista_CleanupSnapshotEntries(uint32 oldest_completed_generation);
pthread_rwlock_t *GetVistaSnapshotLock(void);
#endif
