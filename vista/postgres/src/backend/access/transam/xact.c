...
StartTransactionCommand(void)
{
	...
	switch (s->blockState)
	{
		...
		case TBLOCK_DEFAULT:
			...
			#ifdef VISTA
	// VISTA: for OLAP snapshot setting
	if (!FirstSnapshotSet && session_workload == 1 && vista_BufCtl->ctl.global_generation > 1 && IsUnderPostmaster && IsNormalProcessingMode())
	{
		int oldVal, cur_gen, result;
		bool is_flushing;
		vista_BufferPoolControl *ctl = vista_GetBufferCtl(vista_BufCtl);
		for (;;)
		{
			oldVal = vista_seqlock_read_begin(&ctl->ctl_seqlock);
			cur_gen = ctl->global_generation;
		...
		{
			Assert(cur_gen != 1);
			result = vista_LoadGenerationSnapshot(cur_gen - 2);
			elog(LOG, "applied snapshot %u, current generation %u (now flushing)", cur_gen -2, cur_gen);
		} else
		{
			result = vista_LoadGenerationSnapshot(cur_gen - 1);
			elog(LOG, "applied snapshot %u, current generation %u (flushing done)", cur_gen -1, cur_gen);
		}
	
		Assert(!result); // should return 0; in error, returns -1
...
#ifdef VISTA
			if (!vista_BufCtl->ctl.initial_snapshot_ready && IsUnderPostmaster && IsNormalProcessingMode())
			{
				vista_StoreGenerationSnapshot(0);
				vista_BufCtl->ctl.initial_snapshot_ready = true;
			}
#endif
			break;
