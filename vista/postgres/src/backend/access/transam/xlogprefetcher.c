	...
	for (;;)
	{
		...
		while (prefetcher->next_block_id <= record->max_block_id)
		{
			...
			*lsn = record->lsn;
			...
			// (VISTAXXX) this might be dangerous. I assume that this XLOG accesses all use the metadata pool.(0)
#ifdef VISTA
            result = vista_PrefetchSharedBuffer(0, reln, block->forknum, block->blkno);
#else
