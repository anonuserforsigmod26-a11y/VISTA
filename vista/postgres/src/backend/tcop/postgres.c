...
unsigned long long buffer_wait_time;
			...
			case 'Q':			/* simple query */
				{
					...
					{
					if (mkdir("results", 0776) == -1 && errno != EEXIST)
					{
						fprintf(stderr, "directory create error: %s\n", strerror(errno));
					}

					file_path = (char*) palloc(sizeof(char) * 512);
					output_msg = (char*) palloc(sizeof(char) * 512);
					memset(file_path, 0, sizeof(char) * 512);
					memset(output_msg, 0, sizeof(char) * 512);

					char* appname = MyProcPort ? MyProcPort->application_name : NULL;
								

					sprintf(file_path, "results/result_%s.out", appname);
					if((fd = open(file_path, O_WRONLY | O_APPEND | O_CREAT, 0644)) < 0)
					{
						fprintf(stderr, "file open error: %s\n", strerror(errno));
					}
					buffer_wait_time = 0ULL;
					hit_count = 0;
					read_count = 0;
					}
					#endif
					...
					if (MyBackendType == B_BACKEND)
					{
						sprintf(output_msg, "%d,%.6lf,%lu,%lu\n", getpid(), buffer_wait_time / 1e6, hit_count, read_count);
						if (write(fd, output_msg, strlen(output_msg)) < 0)
						{
