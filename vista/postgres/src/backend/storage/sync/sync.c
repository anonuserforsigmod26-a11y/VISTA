...
#ifdef VISTA
/*
 * Does the same thing as InitSync, but is called from the flush worker.
 * We need this because the flush worker is under postmaster and not a 
 * checkpointer.
 */
void vista_InitSyncForFlushWorker(MemoryContext worker_context)
{
	/*
	 * Create pending-operations hashtable for flush worker.
	 */
	HASHCTL		hash_ctl;
	...
	/*
		...
		* practice.
		*/
	pendingOpsCxt = AllocSetContextCreate(worker_context,
											"Pending ops context for Flush Worker",
											ALLOCSET_DEFAULT_SIZES);
	MemoryContextAllowInCriticalSection(pendingOpsCxt, true);
	...
	hash_ctl.entrysize = sizeof(PendingFsyncEntry);
	hash_ctl.hcxt = pendingOpsCxt;
	pendingOps = hash_create("Pending Ops Table for Flush Worker",
								100L,
								&hash_ctl,
								HASH_ELEM | HASH_BLOBS | HASH_CONTEXT);
	pendingUnlinks = NIL;
