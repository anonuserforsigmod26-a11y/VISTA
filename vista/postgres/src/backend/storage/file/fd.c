...
#ifdef VISTA
/*
 * vista_FileGetRawDesc - get the raw OS file descriptor for a virtual file
 *
 * This is intended for use with APIs that need a raw file descriptor,
 * such as io_uring. The caller must ensure that the file is kept open
 * for the duration of its use.
 */
int
vista_FileGetRawDesc(File file)
{
	Assert(FileIsValid(file));
	...
	{
		/*
		 * This shouldn't happen in our use case, as the file should have
		 * been opened by _mdfd_getseg.
		 */
		elog(ERROR, "file is not open");
	}
