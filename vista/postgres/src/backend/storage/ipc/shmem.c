...
#ifdef VISTA
void * vista_ShmemAlloc(Size size, BufferPoolId pool_id)
{
	if (pool_id < 0) 
	{
		return ShmemAllocNoError(size);
	} else if (pool_id == N_POOLS) {
		return vista_shmem_alloc(&vista_ShmemAllocator, size);
	}
	else {
		return vista_vmseg_alloc(&vista_VmSegAllocatorPerPool[pool_id], size);
	}
}
#endif
...
#ifdef VISTA
	if (infoP->is_buffer_table) {
		if (infoP->pool_id == N_POOLS) {
			location = vista_shmem_alloc(&vista_ShmemAllocator,
										hash_get_shared_size(infoP, hash_flags));
			if (location == NULL)
				ereport(FATAL, errmsg("vista_shmem_alloc failed for snapshot hash table"));
		} else {
			location = vista_vmseg_alloc(&vista_VmSegAllocatorPerPool[infoP->pool_id],
											hash_get_shared_size(infoP, hash_flags));
			if (location == NULL)
				ereport(FATAL, errmsg("vista_vmseg_alloc failed for buffer hash table %d", infoP->pool_id));
		}
		found = false;
	} else {
