...
{
#ifdef VISTA
    pthread_rwlock_t * vista_snapshot_lock = GetVistaSnapshotLock();
    if (pthread_rwlock_destroy(vista_snapshot_lock) != 0)
    {
        fprintf(stderr, "proc_exit: snapshot lock destroy failed.\n");
    }
    if (vista_unregister(&vista_shmem_addrs, 1) != 0)
    {
        fprintf(stderr, "proc_exit: vista_unregister failed.\n");
    }
#endif
