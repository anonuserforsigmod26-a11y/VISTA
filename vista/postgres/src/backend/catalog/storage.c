...
#ifdef VISTA
SMgrRelation
vista_RelationCreateStorage(Relation rel, RelFileNode rnode, char relpersistence,
					  bool register_delete)
{
	SMgrRelation srel;
	BackendId	backend;
	bool		needs_wal;
	// char* poolhint_page; // VISTA
	// int poolhint; // VISTA
	// Size hdrsz; // VISTA
	...
	// // VISTA: create another fork for the pool hint
	// smgrcreate(srel, POOLHINT_FORKNUM, false);
	// if (needs_wal)
	// 	log_smgrcreate(&srel->smgr_rnode.node, POOLHINT_FORKNUM);
	
	// poolhint_page = (char*) palloc0(BLCKSZ);
	// PageInit((Page) poolhint_page, BLCKSZ, 0);
    // hdrsz = MAXALIGN(SizeOfPageHeaderData);

    // poolhint = 0;
	// if (rel->rd_rel->relkind != RELKIND_RELATION || RelationGetNamespace(rel) == PG_CATALOG_NAMESPACE)
    //     poolhint = 1;
	// poolhint_page[hdrsz] = (char) poolhint;
	// smgrextend(srel, POOLHINT_FORKNUM, 0, poolhint_page, false);
	// pfree(poolhint_page);
