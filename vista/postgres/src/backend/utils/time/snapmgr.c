					   ...
					   int sourcepid, PGPROC *sourceproc)
{
	 ...
	 */
// VISTA: we skip this check since we do not care whether the txn is alive
// In fact, we have to install the snapshot of an already finished txn.
#ifdef VISTA
	MyProc->xmin = CurrentSnapshot->xmin;
	TransactionXmin = CurrentSnapshot->xmin;
#else
	 if (sourceproc != NULL)
	{
...
ExportSnapshot(Snapshot snapshot)
{
	 ...
	 * running.
	 */
    // VISTA: since we do not care if the txn is alive, we skip this check.
#ifdef VISTA
#else
	if (IsSubTransaction())
		ereport(ERROR,
