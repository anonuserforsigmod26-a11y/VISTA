...
struct HTAB
{
	...
	int			sshift;			/* segment shift = log2(ssize) */
#ifdef VISTA
	bool		is_buffer_table; /* is this a buffer table? */
	BufferPoolId pool_id; /* pool id for the buffer table */
#endif
};
...
hash_create(const char *tabname, long nelem, const HASHCTL *info, int flags)
{
	...
	HASHHDR    *hctl;
#ifdef VISTA
	vista_hash_table_offsets *hash_offsets;
#endif
	...
	if (flags & HASH_SHARED_MEM)
	{
		...
		hashp->isshared = true;
#ifdef VISTA
		hashp->is_buffer_table = info->is_buffer_table;
		hashp->pool_id = info->pool_id;
#endif
	...
	{
#ifdef VISTA
		if (hashp->is_buffer_table)
			hashp->hctl = (HASHHDR *) vista_ShmemAlloc(sizeof(HASHHDR), hashp->pool_id);
		else 
    ...
    if (hashp->is_buffer_table)
    {
        if (hashp->pool_id == N_POOLS)
        {
            vista_hash_table_offsets *hash_offsets = &vista_BufOffsets->snapshot_hash_offset;
            hash_offsets->hctl_offset = (uint32_t) ((char *) hashp->hctl - (char *) vista_ShmemAllocator.ShmemBase);
            hash_offsets->dir_offset = (uint32_t) ((char *) hashp->dir - (char *) vista_ShmemAllocator.ShmemBase);
            hash_offsets->keysize = hashp->keysize;
            hash_offsets->ssize = hashp->ssize;
            hash_offsets->sshift = hashp->sshift;
        }
        else
        {
            vista_hash_table_offsets *hash_offsets = &vista_BufOffsets->buf_hash_offsets[hashp->pool_id];
            hash_offsets->hctl_offset = (uint32_t) ((char *) hashp->hctl - (char *) vista_VmSegAllocatorPerPool[hashp->pool_id].VmsegBase);
            hash_offsets->dir_offset = (uint32_t) ((char *) hashp->dir - (char *) vista_VmSegAllocatorPerPool[hashp->pool_id].VmsegBase);
            hash_offsets->keysize = hashp->keysize;
            hash_offsets->ssize = hashp->ssize;
            hash_offsets->sshift = hashp->sshift;
        }
...
#ifdef VISTA
		if (hashp->is_buffer_table) {
			hashp->dir = (HASHSEGMENT *) vista_ShmemAlloc(hctl->dsize * sizeof(HASHSEGMENT), hashp->pool_id);
		} else {
...
dir_realloc(HTAB *hashp)
{
	...
	if (hashp->is_buffer_table)
	{
		p = (HASHSEGMENT *) vista_ShmemAlloc((Size) new_dirsize, hashp->pool_id);
	}
	else
	{
...
#ifdef VISTA
	if (hashp->is_buffer_table) {
		segp = (HASHSEGMENT) vista_ShmemAlloc(sizeof(HASHBUCKET) * hashp->ssize, hashp->pool_id);
	} else {
...
element_alloc(HTAB *hashp, int nelem, int freelist_idx)
{
	...
	if (hashp->is_buffer_table)
	{
		firstElement = (HASHELEMENT *) vista_ShmemAlloc(nelem * elementSize, hashp->pool_id);
	} else {
