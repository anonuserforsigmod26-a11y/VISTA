...
#ifdef VISTA
#define StartVistaFlushWorker() StartChildProcess(VistaFlushProcess)
#endif
...
#ifdef VISTA
    if (VistaFlushWorkerPID == 0)
        VistaFlushWorkerPID = StartVistaFlushWorker();
#endif
...
#ifdef VISTA
        if (VistaFlushWorkerPID != 0)
            signal_child(VistaFlushWorkerPID, SIGHUP);
#endif
...
#ifdef VISTA
		if (pid == VistaFlushWorkerPID)
		{
			VistaFlushWorkerPID = 0;
			if (!EXIT_STATUS_0(exitstatus))
				HandleChildCrash(pid, exitstatus,
								 _("Vista Flush Worker process"));
			continue;
		}
#endif        
		/*
...
HandleChildCrash(int pid, int exitstatus, const char *procname)
{
	...
	if (pid == VistaFlushWorkerPID)
		VistaFlushWorkerPID = 0;
	else if (VistaFlushWorkerPID != 0 && take_action)
	{
		ereport(DEBUG2,
				(errmsg_internal("sending %s to process %d",
								 (SendStop ? "SIGSTOP" : "SIGQUIT"),
								 (int) VistaFlushWorkerPID)));
		signal_child(VistaFlushWorkerPID, (SendStop ? SIGSTOP : SIGQUIT));
	}
...
#ifdef VISTA
        if (VistaFlushWorkerPID != 0)
            signal_child(VistaFlushWorkerPID, SIGTERM);
#endif
...
#ifdef VISTA
    if (VistaFlushWorkerPID != 0)
        signal_child(VistaFlushWorkerPID, signal);
#endif
}
	...
	if (pid < 0)
	{
		...
		switch (type)
		{
			...
			case WalReceiverProcess:
				...
				break;
#ifdef VISTA
            case VistaFlushProcess:
                ereport(LOG,
                        (errmsg("could not fork Vista Flush Process: %m")));
                break;
#endif
			default:
				ereport(LOG,
